...
type BoardState = {
  boards: Board[];
  loading: boolean;
  fetchBoards: (workspace?: string) => Promise<void>;
...
export const useBoardStore = create<BoardState>((set) => ({
  ...
  fetchBoards: async (workspace?: string) => {
    const fetchId = ++_latestBoardsFetchId;
    try {
      set({ loading: true });
      console.debug('useBoardStore.fetchBoards start', { workspace, fetchId });
      const boards = await boardService.list(workspace);
